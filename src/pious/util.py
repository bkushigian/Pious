...
for canonical in PIO_HAND_ORDER:
    reversed = canonical[2:] + canonical[:2]
    FULL_COMBO_HAND_ORDER_CANONICAL[canonical] = canonical
    FULL_COMBO_HAND_ORDER_CANONICAL[reversed] = canonical


def canonicalize_full_combo(full_combo):
    """
    Each combo can be written in two ways by swapping card order.
    We ensure that the card is in the correct order such that:

    1. if one card's rank is higher than the other, that card is first
    2. if both cards have the same rank then spades comes before hearts comes
       before clubs comes before diamonds
    """
    return FULL_COMBO_HAND_ORDER_CANONICAL[full_combo]


def get_pio_combo_index(full_combo):
    """
    Canonicalize the combo and return the index in PioSolver's hand order
    """
...
POCKET_PAIR_COMBO_SUITS = tuple(
    "".join((s1, s2)) for (s1, s2) in combinations(SUIT_ORDER, r=2)
)
SUITED_COMBO_SUITS = ("ss", "hh", "cc", "dd")


def is_preflop_combo(c: str):
    if len(c) == 2:
        return c[0] == c[1] and c[0] in ranks
    elif len(c) == 3:
        return c[0] != c[1] and c[0] in ranks and c[1] in ranks and c[2] in "so"
    return False


def is_full_combo(c: str):
    return FULL_COMBO_HAND_ORDER_CANONICAL.get(c, False)


def combo_as_full_combos(combo: str) -> Tuple[str]:
    """
    Take a combo, either a preflop combo or a full combo, and return a tuple of
    all full combos.
    ...
    ('AhTd',)
    """
    if is_full_combo(combo):
        return (combo,)
    elif is_preflop_combo(combo):
        return preflop_combo_to_full_combos(combo)
    else:
        raise ValueError(f"Illegal combo {combo}")
...
def full_combo_to_preflop_combo(full_combo: str) -> str:
    if len(full_combo) != 4:
        raise ValueError(
            f"Full combo must be length 4 of the form {{RANK1}}{{SUIT1}}{{RANK2}}{{SUIT2}}: {full_combo}"
        )
    r1, s1, r2, s2 = full_combo
    r1 = get_rank_as_int(r1)
    r2 = get_rank_as_int(r2)
    # Ensure that r1 and r2 are in the correct 'canonical' order with r1 > r2.
    # Thus, 2c3h should become 3h2c
    if r1 < r2:
        r2, r1 = r1, r2  # We don't care about suits here
    base = f"{get_rank_as_str(r1)}{get_rank_as_str(r2)}"
    if r1 > r2:
        if s1 == s2:
            return f"{base}s"
        else:
            return f"{base}o"
    if s1 == s2:
        raise ValueError(f"Pocket pairs cannot have the same suit: {full_combo}")
    return base


def preflop_combo_to_full_combos(preflop_combo: str) -> Tuple[str]:
    if len(preflop_combo) < 2 or len(preflop_combo) > 3:
        raise ValueError(
            f"Preflop combos must be of the form 'XX' for pocket pairs, 'XYs' for suited combos, or 'XYo' for offsuit combos"
        )
    r1, r2 = preflop_combo[:2]

    if len(preflop_combo) == 3:
        s_or_o = preflop_combo[2]
    else:
        s_or_o = None

    if s_or_o is not None and s_or_o not in "so":
        raise ValueError(f"Suitedness specifier '{s_or_o}' must be either 's' or 'o'")

    if r1 == r2:  # Pocket Pairs
        if s_or_o:
            raise ValueError(f"Pocket pairs cannot have a suitedness qualifier")
        return tuple([f"{r1}{s1}{r2}{s2}" for (s1, s2) in POCKET_PAIR_COMBO_SUITS])

    if s_or_o is None:
        raise ValueError(
            f"For different ranks {r1} and {r2} you must specify suited ('s') or offsuit ('o'): {preflop_combo}"
        )
    if s_or_o == "o":
        return tuple([f"{r1}{s1}{r2}{s2}" for (s1, s2) in OFFSUIT_COMBO_SUITS])
    if s_or_o == "s":
        return tuple([f"{r1}{s1}{r2}{s2}" for (s1, s2) in SUITED_COMBO_SUITS])
    raise RuntimeError("Unreachable")


def combo_name_from_combo_index(idx: int):
    if idx < 0 or idx >= NUM_COMBOS:
        raise ValueError(
            f"Index must be between 0 (inclusive) and {NUM_COMBOS} (exclusive)"
        )
...
def get_rank_as_int(r):
    if isinstance(r, int) and 2 <= r <= 14:
        return r
    if r in ranks:
        return ranks[r]
    raise ValueError(
        f"Illegal rank value {r}: must be an int between 2 and 14 (inclusive) or a single char in '23456789TJQKA'"
    )


def get_rank_as_str(r):
    if isinstance(r, str) and r in ranks:
        if r == "10":
            r = "T"
        return r
    if r in ranks_rev:
